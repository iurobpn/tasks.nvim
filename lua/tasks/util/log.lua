...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.tasks.config ~= nil and not _G.tasks.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[tasks.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "tasks.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h tasks.options` to read more."
        )
    end
end
